...
def main():
    ...
    error_base1 = ((data_base1.test_latent_reps - pred_base1) ** 2).sum(1).cpu().numpy().squeeze(0)
    pred_sn1 = data_sn1.latent_approx()
    error_sn1 = ((data_sn1.test_latent_reps - pred_sn1) ** 2).sum(1).cpu().numpy().squeeze(0)
    pred_base2 = data_base2.latent_approx()
    error_base2 = ((data_base2.test_latent_reps - pred_base2) ** 2).sum(1).cpu().numpy().squeeze(0)
    pred_sn2 = data_sn2.latent_approx()
    error_sn2 = ((data_sn2.test_latent_reps - pred_sn2) ** 2).sum(1).cpu().numpy().squeeze(0)
